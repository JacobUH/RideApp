...
struct RouteMapView: UIViewRepresentable {
    ...
    private func fetchAddress(from coordinate: CLLocationCoordinate2D, completion: @escaping (String?) -> Void) {
        ...
        CLGeocoder().reverseGeocodeLocation(location) { placemarks, error in
            if let error = error {
                print("DEBUG: Failed to fetch address - \(error.localizedDescription)")
                completion(nil)
                return
            }
...
extension RouteMapView {
    class MapCoordinator: NSObject, MKMapViewDelegate {
        ...
        func mapView(_ mapView: MKMapView, didUpdate userLocation: MKUserLocation) {
            ...
            CLGeocoder().reverseGeocodeLocation(location) { placemarks, error in
                if let error = error {
                    print("DEBUG: Failed to fetch origin address - \(error.localizedDescription)")
                    return
                }
                if let address = placemarks?.first?.name {
                    self.parent.originAddress = address
                    self.parent.onAddressesUpdated?(address, self.parent.destinationAddress)
                }
            }
        }
